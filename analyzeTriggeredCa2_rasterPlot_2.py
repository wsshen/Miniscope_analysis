...
dir = tkinter.filedialog.askdirectory(initialdir="/media/watson/UbuntuHDD/feng_Xin/Xin/Miniscope/4914202252023/06082023/13_20_16")
labviewFile = 'LabVIEWStims_06_08_2023_13_20_10.txt'

with open(dir+os.sep+'traces_registered.pickle', 'rb') as f:
    cellTraces =pickle.load(f)
with open(dir + os.sep + 'idx_registered.pickle', 'rb') as f:
...
pumpDelays = readTabTxtFile(dir+os.sep+labviewFile, 'int')
...
ca_pos_temp_allPumps = findAllActIndices(trimmed_pumpTrigTime, startTime, caTime, timeWindow)
...
pumpVector = np.zeros(len(tVector))
pumpVector[ca_pos_temp_allPumps-np.tile(startInd,len(ca_pos_temp_allPumps))]=1*50
...
for i in range(len(valid_idx)):
    ...
    plt.plot(tVector, pumpVector, color='g', linewidth=1)
    plt.text(0,10,str(valid_idx[i]))
    ca_raw_traces.append(plot_ca)
plt.savefig(dir+os.sep+plotSavingFolder+os.sep+'rasterPlot_2.eps',format='eps')
print("done")
...
plt.savefig(dir+os.sep+plotSavingFolder+os.sep+'averagePlot_2.eps', format='eps')
