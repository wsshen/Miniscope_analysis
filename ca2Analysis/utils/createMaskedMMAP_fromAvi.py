...
dir = "/media/watson/UbuntuHDD/feng_Xin/Xin/Miniscope/4914202252023/05212023/10_03_47/My_V4_miniscope"
# # os.chdir(dir)
...
def draw_circle(event, x, y, flags, param):
    ...
    # if the left mouse button was released, set the drawing flag to False
        ...
        drawing = False
        params = [ix,iy,x,y]
        cv2.circle(enhanced_image, calc_center(ix, iy,x,y), calc_radius(ix, iy,x,y), (255, 0, 0), -1)
...
while hasFrame:
    gray_frame = cv2.cvtColor(frame,cv2.COLOR_BGR2GRAY)
    masked = cv2.bitwise_and(gray_frame, gray_frame, mask=circle)
    newFrames.append(masked)
    hasFrame, frame = v.read()
dims = [600,600]
newFrames = np.transpose(newFrames, list(range(1, len(dims) + 1)) + [0])
newFrames = np.reshape(newFrames, (np.prod(dims), 1000), order='F')
newFrames = np.ascontiguousarray(newFrames, dtype=np.float32) + np.float32(0.0001) + np.float32(0)
    # cv2.namedWindow('masked',cv2.WINDOW_NORMAL)
    # cv2.imshow('masked',masked)
    # cv2.waitKey(0)

new_fname = '/home/watson/Documents/caiman_fromCluster/memmap__d1_600_d2_600_d3_1_order_C_frames_1000.mmap'
